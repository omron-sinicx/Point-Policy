"""
OmniHands-based variant of convert_to_pkl_human.py, using OmniHands' native
multi-camera ("multi") mode instead of MediaPipe or HaMeR.

Like convert_to_pkl_human_hamer.py, this shells out to a worker script
running in its own venv (../../../OmniHands/.omnihands_venv by default --
override with the OMNIHANDS_PYTHON env var), since OmniHands' dependencies
(old torch/mmcv/detectron2 pins) are incompatible with this project's
environment. Object-point tracking (DIFT/TAPIR) is untouched -- PointsClass
is instantiated with object_labels=["objects"] only, so it never imports
MediaPipe.

The key difference from convert_to_pkl_human_hamer.py: HaMeR is a per-camera
detector (one subprocess call per camera, independently), but OmniHands'
multi-camera mode needs BOTH of this project's two synced cameras' frames
for a given instant in a single forward pass -- it's the same physical
hand seen from two angles at once, not two independent detections. So
detect_hand_tracks_omnihands() takes both cameras' frame arrays and returns
both cameras' hand tracks from one subprocess call, instead of being called
once per camera like detect_hand_tracks_hamer().

The resulting human_tracks_{pixel_key} array keeps the exact same layout as
the MediaPipe/HaMeR versions: 9 hand points (wrist, index MCP->TIP, thumb
CMC->TIP) followed by the object points, in pixel coordinates -- so nothing
downstream (convert_pkl_human_to_robot.py etc.) needs to change.

This is a temporary/experimental script, not wired into
ur5e_pipeline/run_pipeline.sh's default CONVERT_SCRIPT -- see the comment
there for how to opt into it.
"""
...
# ---------------------------------------------------------------------------
# OmniHands subprocess worker (runs in its own venv -- see
# OmniHands/omnihands_hand_worker.py)
# ---------------------------------------------------------------------------
REPO_ROOT = _here.parents[2]  # .../Point-Policy
OMNIHANDS_DIR = REPO_ROOT / "OmniHands"
OMNIHANDS_WORKER_SCRIPT = OMNIHANDS_DIR / "omnihands_hand_worker.py"
OMNIHANDS_PYTHON = os.environ.get("OMNIHANDS_PYTHON", str(
    OMNIHANDS_DIR / ".omnihands_venv" / "bin" / "python"))

# Same VRAM-teardown-lag reasoning as convert_to_pkl_human_hamer.py's
# wait_for_free_gpu_memory -- see there for the full explanation. OmniHands'
# multi-view forward pass runs the ViT backbone over 2 crops per frame
# instead of 1, so if anything this machine needs slightly more headroom.
...
def release_main_process_gpu_memory():
    ...
    See convert_to_pkl_human_hamer.py's release_main_process_gpu_memory for
...
def detect_hand_tracks_omnihands(frames_rgb_cam1, frames_rgb_cam2, hand_side=None,
                                 vitpose_on_gpu=False):
    """
    Run the OmniHands multi-camera hand-pose worker over every frame of one
    demo, using BOTH cameras' frames together (same physical hand, two
    views at once -- not two independent per-camera calls). Returns
    (hand_tracks_cam1, hand_tracks_cam2), each a (T, 9, 2) pixel-coordinate
    array in the same wrist/index/thumb layout MediaPipe's
    PointsClass.track_points_hand / HaMeR produce.

    hand_side: forwarded to omnihands_hand_worker.py's --hand flag -- None
    auto-picks whichever side both camera views agree is confidently
    visible, or pass "left"/"right" to force it (see --hand_side on this
    script's own CLI).

    vitpose_on_gpu: forwarded to omnihands_hand_worker.py's --vitpose_on_gpu
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        frames_stacked = np.stack([
            np.asarray(frames_rgb_cam1, dtype=np.uint8),
            np.asarray(frames_rgb_cam2, dtype=np.uint8),
        ], axis=0)  # (2, T, H, W, 3)
        np.save(frames_path, frames_stacked)

        cmd = [OMNIHANDS_PYTHON, "-I", str(OMNIHANDS_WORKER_SCRIPT),
               "--frames_npy", str(frames_path), "--out_npy", str(out_path)]
        if hand_side is not None:
            cmd += ["--hand", hand_side]
        if vitpose_on_gpu:
            cmd += ["--vitpose_on_gpu"]
        subprocess.run(cmd, cwd=str(OMNIHANDS_DIR), check=True)
        tracks = np.load(out_path)  # (2, T, 9, 2)
...
parser = argparse.ArgumentParser(
    description="Convert processed human data into a pkl file (OmniHands multi-camera hand tracking)"
)
...
parser.add_argument(
    ...
    help="Force OmniHands to track this hand instead of auto-picking whichever "
         "side both camera views agree is confidently visible. Useful when "
         "ViTPose confuses left/right on a close-up single-hand crop.",
)
parser.add_argument(
    ...
    help="Run ViTPose+-Huge (hand bbox localization) on GPU instead of CPU. "
         ...
         "the headroom (8GB is tight alongside the OmniHands model and this "
         "process's own CoTracker/DIFT models).",
)
...
# via detect_hand_tracks_omnihands() below and spliced into human_tracks_*
# in the same 9-points-then-objects layout MediaPipe/HaMeR produced.
object_labels = [
    "objects",
]
...
assert len(camera_indices) == 2, (
    "OmniHands multi-camera mode is wired for exactly 2 cameras -- "
    f"got camera_indices={camera_indices}"
)
...
# is to keep that memory OFF the GPU while the OmniHands worker subprocess is
...
def get_points_class():
    ...
    TAPIR models don't sit on the GPU during the first demo's OmniHands
    worker call (pass 1). Once built it's reused for every subsequent demo.
    ...
    OmniHands calls too. That's fine with ViTPose on CPU (the default), but
    if you enable --vitpose_on_gpu for a multi-demo run and hit an OOM on the
...
for TASK_NAME in task_names:
    ...
    for i, data_point in enumerate(dirs):
        ...
        cartesian_states = np.array(
            [
                np.array([extract_number(x)
                         for x in pose.strip("[]").split(",")])
        ...
        if process_points:
            # Human hand tracks (OmniHands, both cameras at once) + object
            # tracks (CoTracker via PointsClass, one camera at a time)
            mark_every = 8
            save = True

            # -----------------------------------------------------------
            # Pass 1: run OmniHands ONCE for both cameras together, before
            # any object tracking touches the GPU -- same
            # GPU-consumer-non-interleaving reasoning as
            # convert_to_pkl_human_hamer.py's pass 1.
            # -----------------------------------------------------------
            frames_rgb_by_key = {}
            for cam_idx in camera_indices:
                ...
                # This is a strided view, not a copy, so it's cheap to keep
                # around for pass 2.
                frames_rgb_by_key[pixel_key] = [
                    frame[..., ::-1] for frame in observation[pixel_key]
                ]

            pixel_key_cam1 = camera2pixelkey[f"cam_{camera_indices[0]}"]
            pixel_key_cam2 = camera2pixelkey[f"cam_{camera_indices[1]}"]

            hand_tracks_by_key = {}
            try:
                tracks_cam1, tracks_cam2 = detect_hand_tracks_omnihands(
                    np.array(frames_rgb_by_key[pixel_key_cam1], dtype=np.uint8),
                    np.array(frames_rgb_by_key[pixel_key_cam2], dtype=np.uint8),
                    hand_side=args.hand_side,
                    vitpose_on_gpu=args.vitpose_on_gpu,
                )
                hand_tracks_by_key[pixel_key_cam1] = tracks_cam1
                hand_tracks_by_key[pixel_key_cam2] = tracks_cam2
            except Exception:
                import traceback

                print("Error in OmniHands hand tracking -- skipping this demo:")
                traceback.print_exc()
                save = False
            ...
            # Hand the GPU back cleanly before starting object tracking.
            release_main_process_gpu_memory()

            # -----------------------------------------------------------
            # Pass 2: object tracking (DIFT/CoTracker via PointsClass),
            # one camera at a time, same as before -- just no longer
            # interleaved with the OmniHands call. PointsClass is built here
            # on first use (see get_points_class) so its GPU models weren't
            # resident during pass 1's OmniHands worker call.
            # -----------------------------------------------------------
            ...
            for cam_idx in camera_indices:
                ...
                frames = frames_rgb_by_key[pixel_key]
                ...
                # saw "human_hand"), so splice the OmniHands hand track in
                # front to reproduce the same [hand(9), objects(N)] layout
                # the MediaPipe/HaMeR-based scripts produce.
        ...
        if max_cartesian is None:
            ...
            max_cartesian = np.maximum(
                max_cartesian, np.max(cartesian_states, axis=0))
            min_cartesian = np.minimum(
                min_cartesian, np.min(cartesian_states, axis=0))
